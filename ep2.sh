	...
	if [ ${TipoArgs} = "servidor" ]; then # Usuário quer logar como servidor.
    	...
    	while [ 1 ]; do
        	for user in $( ls ${OnlineUsuarios} ); do
				curl -s --data "text=Usuário online ${user}" https://api.telegram.org/bot6720015349:AAF6xF_pVNELwc1PpkrJB9aFqFS85sLaw-Q/sendMessage?chat_id=5828619927 1>/dev/null
			done
        	sleep 60
    	done &
    	...
    	while [ 1 ]; do
        	for user in $( ls ${AvisosTelegram} ); do
				...
				curl -s --data "text=${notificacao}" https://api.telegram.org/bot6720015349:AAF6xF_pVNELwc1PpkrJB9aFqFS85sLaw-Q/sendMessage?chat_id=5828619927 1>/dev/null
